...
class ImportKDBAI(ImportVDF):
    ...
    def upsert_data(self):
        ...
        parquet_columns = [{"name": field.name, "type": str(field.type)} for field in parquet_schema]
        ...
        if self.index.upper() not in allowed_vector_types:
            raise ValueError(f"Invalid vectorIndex type: {self.index}. "
        ...
        for i in range(0, df.shape[0], batch_size):
            ...
            try:
                table.insert(chunk)
                print(f"Inserted {min(i + batch_size, df.shape[0])} out of {df.shape[0]} rows.")
