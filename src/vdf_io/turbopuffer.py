...
from vdf_io.util import standardize_metric, clean_documents, set_arg_from_input
...
def export_vdb(args):
    ...
    if index_names is None:
        ...
        index_names = input("Enter the index names to export (comma-separated), or leave empty to export all: ").split(",")
...
def import_vdb(args, input_data):
    ...
    if distance_metric is None:
        distance_metric = input("Enter the distance metric to use (cosine_distance, euclidean_distance, dot_product): ")
    ...
    for doc in input_data:
        if len(batch) >= batch_size:
            ns.upsert(
                ...
                attributes={k: [d.get(k) for d in batch] for k in batch[0].keys() if k not in ["id", "vector"]},
