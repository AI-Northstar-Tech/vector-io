                ...
                for object in objects['objects']:
                    if cross_ref_name in object['properties']:
                        for refs in object['properties'][cross_ref_name]:
                            ...
                            if match:
                                uuid = match.group(1)
                                obj = self.weaviate_client.data_object.get_by_id(class_name=cross_ref_class_name, uuid=uuid)
                                if obj is not None:
                                    data_dict = {}
                                    data_dict['main_uuid'] = object['id']
                                    data_dict['uuid'] = obj['id']
                                    for property_name in property_names_dict[cross_ref_class_name]:
                                        if property_name in obj['properties']:
                                            data_dict[property_name] = obj['properties'][property_name]
                                        else:
                                            data_dict[property_name] = ''
                                    data_tuple = ()
                                    for property in data_dict.values():
